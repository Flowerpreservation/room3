...
class Home extends Component {
    ...
    showName() {
        ...
        $.ajax({
            ...
            success(res) {
                console.log(res)
                if (res.code === 200 || res.code === 0) {
                    if (res.remarkList && res.remarkList !== '') {
                        that.setState({
    ...
    render() {
        ...
        let roomAll = dataRoom.length ? dataRoom.map((item, index) => {
            return <li key={index} onClick={this.addSickbed.bind(this, item.id, item.roomName)}>{item.roomName}</li>
        }) : <li >暂无病房</li>;

        // 病区
        let listAll = dataList.length ? dataList.map((item, index) => {
            return <li key={index}>
                <p onClick={this.addRooms.bind(this, item.id, item.areaName)}>{item.areaName}</p>
        ...
        let bedAll = dataBed.length ? dataBed.map((item, index) => {
            return <div className="home-card" key={index} onClick={this.toDetail.bind(this, item.id)}>
                <p className="home-card-tit">{item.bedName}</p>
                <p className="home-last">8小时前进行录音</p>
                <div className="home-operate">
                    <img src={require('../images/write.png')} alt="备注" onClick={this.loadRemark.bind(this, item.id)} />
        ...
        let detail_bedAll = dataBed.length ? dataBed.map((item, index) => {
            return <div className="detail-card" key={index} onClick={this.tapeBars.bind(this, item.id)}>
                <p className="home-card-tit">{item.bedName}</p>
                <p className="home-last">8小时前进行录音</p>
                <div className="home-operate">
                    <img src={require('../images/write.png')} alt="备注" onClick={this.loadRemark.bind(this, item.id)} />
